...
from proto.PlatformRegisterReq_pb2 import PlatformRegisterReq
...
def main():
    ...
    # Dump the blob if present
    if msg.platform_register_info:
        blob = msg.platform_register_info
        print(f"\nplatform_register_info raw hex ({len(blob)} bytes): {blob.hex()}")
    else:
        print("\nplatform_register_info field is empty in this request.")
